...
def read_config_file(config_file):
    ...
    try:
        ...
        return dict(column_settings), dict(series_conversion_rules), dict(dataframe_conversion_rules)
...
def process_data_frame(df, column_settings, series_conversion_rules, dataframe_conversion_rules):
...
if __name__ == '__main__':
    ...
    column_settings, series_conversion_rules, dataframe_conversion_rules = read_config_file(config_file)

    input_file = 'offset_flatten_rsm_30_all.csv'
    df = read_csv_file(input_file)

    # 根据列名和转换规则进行 DataFrame 转换
    generated_df = process_data_frame(df, column_settings, series_conversion_rules, dataframe_conversion_rules)
